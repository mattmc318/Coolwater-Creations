...
this.onSettledCallbacks_=null}};var g=new b;d.prototype.settleSameAsPromise_=function(a){var b=this.createResolveAndReject_();a.callWhenSettled_(b.resolve,b.reject)};d.prototype.settleSameAsThenable_=function(a,b){var c=this.createResolveAndReject_();try{a.call(b,c.resolve,c.reject)}catch(m){c.reject(m)}};d.prototype.then=function(a,b){function c(a,b){return"function"==typeof a?function(b){try{e(a(b))}catch(A){f(A)}}:b}var e,f,g=new d(function(a,b){e=a;f=b});this.callWhenSettled_(c(a,e),c(b,f));return g};
...
$(function(){var a=$(".banner"),b=$(".bg"),c=$(".logo"),e=$(".content"),d=e.children(".content-container").children("ul"),g=d.children("li"),f=g.children("a"),k=f.children("button"),l=k.children("svg"),m=f.children("label"),z=$("footer");switch(getCookie("gdpr")){case "":setCookie("gdpr","false");case "false":a.show();break;case "true":a.hide()}a.on("click",".close",function(b){b.preventDefault();a.hide();setCookie("gdpr","true")});var q=!0,v=function(){b.toggleClass("landscape").toggleClass("portrait");
q=!q},h=$(window).width(),p=$(window).height();c.on("click",function(a){a.preventDefault();b.animate({scrollTop:Math.ceil(p-c.outerHeight())},1E3)});f=function(){var a,b,f,u,w,x,y,r,t;return $jscomp.asyncExecutePromiseGeneratorProgram(function(n){1==n.nextAddress&&(h=$(window).width(),p=$(window).height(),a=c.children("h1"),358>h?a.css({"letter-spacing":"normal","font-size":"1.75rem"}):439>h?a.css({"letter-spacing":"normal","font-size":"2.0rem"}):553>h?a.css({"letter-spacing":"normal","font-size":"2.5rem"}):
a.css({"letter-spacing":"6px","font-size":"2.5rem"}),b=0);if(3!=n.nextAddress){if(!(6>b))return n.jumpTo(0);f=c.outerHeight();u=z.outerHeight();w=2*p-f-u;x=f-u;y=Math.max.apply(Math,$jscomp.arrayFromIterable(m.map(function(){return $(this).outerHeight()})));h>p?(d.css({"flex-direction":"row"}),!1===q&&v()):(d.css({"flex-direction":"column"}),!0===q&&v());e.css({height:w+"px","padding-bottom":x+"px"});k.hide();r=Math.min(150,g.first().width()-8,g.first().height()-16-y-8);t=4*r/150;1>t?k.hide():(k.show(),
k.css({height:r+"px",width:r+"px","border-width":Math.ceil(t)+"px"}),l.css({"font-size":t+"em"}));return n.yield(sleep(500),3)}b++;return n.jumpTo(2)})};$(window).on("resize orientationchange",f);f()});
